...
**智能路由引擎**（核心改造）：
- **模型亲和性**：成功的 URL 记住，下次同模型直接复用，不再随机碰运气
- **成功率加权**：URL 选择权重从纯延迟改为 `成功率²/延迟`，烂 URL 被指数压制
- **优先级乘法缩放**：`有效优先级 = 基础优先级 * (0.2 + 0.8 * 成功率)`，成功率低的渠道自动让位，不用手动调优先级
...
- **URL 测试选模型**：单 URL 测试时可选择测哪个模型，不再只能测第一个
- **远程渠道管理**（manage.py）：一条命令完成 下载CSV -> 探活 -> 清洗 -> 上传

## 痛点解决

- **手动调优先级** -> 成功率驱动的动态优先级，烂渠道自动靠边站
- **URL 随机选** -> 模型亲和性 + 成功率加权，找到好 URL 就一直用
- **一个 URL 挂全渠道废** -> 5xx 自动试下一个 URL，不跳渠道
- **死 URL 反复重试** -> 冷却最高 4 小时，别老撞墙
- **手动导出导入 CSV** -> manage.py 一条命令自动化
...
| 能力 | 亮点 | 效果 |
|------|------|------|
| **模型亲和性** | 成功 URL 自动记忆，同模型复用 | 找到好 URL 就一直用，不浪费 |
| **成功率驱动** | URL 权重 = 成功率²/延迟 | 烂 URL 被指数压制 |
| **动态优先级** | 优先级 * 成功率乘法缩放 | 不用手动调优先级了 |
| **智能故障切换** | 5xx 试下一个 URL，不跳渠道 | 一个 URL 挂不影响整个渠道 |
| **模型缺失检测** | unknown provider 不冷却 URL | URL 没某模型不算故障 |
| **一键清冷却** | 渠道列表操作按钮 | 渠道+Key+URL 冷却一键清除 |
| **远程管理** | manage.py 自动登录/下载/上传 | 一条命令管理生产渠道 |
| **性能** | Gin + Sonic JSON | 1000+并发 |
| **多API兼容** | Claude/Gemini/OpenAI | 一套配置走天下 |
| **开箱即用** | 单文件+嵌入式SQLite | 零依赖 |
| **成本限额** | 渠道每日成本+令牌费用上限 | 精细化访问控制 |
| **多URL负载均衡** | 单渠道多URL+成功率加权随机 | 好 URL 自动多分流 |
| **分层定价** | GPT-5.4/Qwen-Plus/Gemini | 超量token自动降档计费 |
...
配合 `manage.py` 脚本实现一键自动化：从服务器下载 CSV -> 探活清洗 -> 上传回去。

```bash
# 设置环境变量（一次就行）
# CCLOAD_SERVER=https://your-server.com
# CCLOAD_PASS=your_admin_password

# 日常维护：探活 + 清洗全部渠道
python manage.py -t all

# 只清洗 Claude 和 Gemini
python manage.py -t claude,gemini

# 添加新 URL（探活后添加）
python manage.py -t claude,gemini --add new.txt

# 只下载 CSV 看看
python manage.py -t all --download-only
```

支持 Claude（分级检测 thinking 能力）、Gemini（真实 chat 请求）、GPT（带 reasoning 测试）三种探活方式。详见 `manage.py --help`。
...
- **冷却管理器**（DRY原则）：
  ...
  - 区分网络错误和HTTP错误的分类策略
  - 内置单Key渠道自动升级逻辑
- **多URL选择器**（URLSelector，增强版）：
  - `url_selector.go`：单渠道多URL智能调度
  - 模型亲和性：成功的 URL 记住，同模型下次直接复用
  - 成功率加权：权重=成功率²/EWMA延迟，烂 URL 被指数压制
  - 智能探索：有已知好 URL 时不盲目探索未知 URL
  - 独立冷却：故障URL指数退避（最高4h），不影响同渠道其他URL
  - 模型缺失检测：`unknown provider for model` 不冷却 URL，试下一个
  - 一键清冷却：管理界面按钮清除渠道所有冷却（渠道+Key+URL）
...
启用 `enable_health_score` 后，系统根据渠道历史成功率动态调整优先级，使用**乘法缩放**：

```
置信度 = min(1.0, 样本量 / health_min_confident_sample)
调整后成功率 = 成功率 * 置信度 + 1.0 * (1 - 置信度)
有效优先级 = 基础优先级 * (0.2 + 0.8 * 调整后成功率)
```

乘法缩放的优势：**能跨越任何静态优先级差距**。不管你设的优先级差多少，成功率低的渠道都会被自动压下去。

**示例**（`health_min_confident_sample = 20`）：

| 渠道 | 基础优先级 | 成功率 | 样本量 | 有效优先级 |
|------|-----------|--------|--------|-----------|
| A | 1200 | 90% | 100 | **1064** |
| B | 1200 | 20% | 80 | **432** |
| C | 1111 | 95% | 50 | **1067** |
...
基础优先级排序：A = B > C > D
**有效优先级排序：C (1067) > A (1064) > D (1000) > B (432)**

- 渠道 B 和 A 基础优先级一样是 1200，但 B 成功率只有 20%，直接被压到 432
- 渠道 C 基础优先级低于 A，但成功率更高，反超 A
- 不用手动调优先级，系统自动按实际表现排序
